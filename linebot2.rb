...
post '/callback' do
  ...
  return 'Bad Request' unless client.validate_signature(body, signature)
  ...
  events.each do |event|
    if event.is_a?(Line::Bot::Event::Message) && event.type == Line::Bot::Event::MessageType::Text
      ...
      # あいまい検索
      matched_line = LINES.keys.find { |name| name.include?(user_text) }
      if matched_line
        redis.incr("count:#{matched_line}") # 人気カウント
        status, detail = fetch_status(LINES[matched_line])
        message = "【#{matched_line}】\n状況: #{status}\n詳細: #{detail}"
      else
        message = "その路線はリストにありません。"
      end
